...
<section class="pt-4 pb-12">
	...
	<p class="mt-6 max-w-2xl text-lg leading-relaxed text-ink-2">
		This lesson walks through how Voyager builds a trip. Four steps: load a model, assemble a prompt from your preferences, draft a day-by-day plan, and rank real places against that draft. Everything runs in your browser.
	</p>
</section>
...
<section class="pb-12">
	<div class="flex flex-col gap-6 text-[17px] leading-relaxed text-ink-2">
		...
		<p>
			Voyager uses <a href="https://webllm.mlc.ai/" class="underline underline-offset-4">WebLLM</a> to load a small language model into the browser through <a href="https://www.w3.org/TR/webgpu/" class="underline underline-offset-4">WebGPU</a>. The wrapper lives at <code class="rounded bg-bone-200 px-1.5 py-0.5 text-[15px]">src/lib/webllm/engine.svelte.js</code>.
		</p>
		<p>
		...
		<p>
			A raw model doesn't know you hate 10am queues. Voyager keeps your preferences locally and folds them into the prompt. The shape of that assembly lives in <code class="rounded bg-bone-200 px-1.5 py-0.5 text-[15px]">src/lib/preferences/contextPrompt.svelte.js</code>.
		</p>
		<p>
		...
		<p>
			A model-only plan is a hallucination factory. Voyager grounds each block against a local place dataset at <code class="rounded bg-bone-200 px-1.5 py-0.5 text-[15px]">src/lib/agent/fixtures/places.json</code>. The agent runtime at <code class="rounded bg-bone-200 px-1.5 py-0.5 text-[15px]">src/lib/agent/index.ts</code> selects and orders the candidates.
		</p>
		<p>
		...
		<p>
			Put together: local model, local preferences, local data, local ranking. No round-trip. No account. No API key. That is the structural promise of Voyager. It is the reason the rest of these lessons exist.
		</p>
	</div>
</section>
