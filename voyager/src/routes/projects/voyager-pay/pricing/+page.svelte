...
<section class="mx-auto max-w-6xl px-6 pb-16">
	...
	<div class="mt-10 flex flex-col gap-6 text-[17px] leading-relaxed text-[var(--register-muted)]">
		<p>
			A Mostro node is an independent operator that bridges fiat and Bitcoin. Each one publishes a profile — region, fiats, methods, fee, HODL liquidity, reputation — as a <code class="rounded bg-[var(--register-card)] px-1.5 py-0.5 text-[15px]">kind:38383</code> event. The customer's wallet reads those profiles, filters by fiat and method, and ranks what remains.
		</p>
		<p>
			Three things compete: <span class="font-display text-[var(--register-text)]">rate</span>, <span class="font-display text-[var(--register-text)]">fee</span>, and <span class="font-display text-[var(--register-text)]">reputation</span>. An operator who charges a fat spread loses the order to a competitor on the next refresh. An operator who fakes reputation loses it on the next settled trade, because reputation is keyed to a public key that has actually settled volume.
		</p>
	</div>
</section>
	...
	<div class="rounded-[32px] bg-[var(--register-card)] p-8 sm:p-12">
		...
		<div class="mt-12 flex flex-col gap-6 rounded-[28px] bg-[var(--register-card)] p-8 sm:flex-row sm:items-center sm:justify-between">
			<div>
				...
				<h3 class="mt-2 font-display text-xl text-[var(--register-text)]">Run a node — anyone can join the federation.</h3>
				<p class="mt-2 text-sm text-[var(--register-muted)]">What a node does, what it costs, what's in it for the operator.</p>
			</div>
